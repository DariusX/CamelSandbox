...
public class FileSplitterRoute extends RouteBuilder {
	...
	public void configure() throws Exception {
		...
		.log(LoggingLevel.INFO, "${id} File to process: ${file:name}")
				.split(body().tokenize("\n")).streaming().choice().when(header("CamelFileName").endsWith(".txt"))
				...
				.when(header("CamelFileName").endsWith(".csv")).unmarshal(bindyFormat).end()
				.bean(new CustomerProcessor(), "processCustomer").log("${id} Done record from: ${file:name}").end()
				.log("${id} Spitting is done").aggregate(new ConstantExpression("Z"), new CustomerAggregator())
